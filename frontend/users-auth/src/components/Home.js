...
function Home() {
  ...
  const [windowSize, setWindowSize] = useState([
    window.innerWidth, //ความกว้างของหน้าจอ
    window.innerHeight, //ความสูงของหน้าจอ
  ]);

  useEffect(() => { //ใช้เพื่อทำการติดตามเหตุการณ์การเปลี่ยนขนาดของหน้าต่าง

    const handleWindowResize = () => {
      setWindowSize([window.innerWidth, window.innerHeight]);
    };

    window.addEventListener("resize", handleWindowResize); //สร้าง event listener ตรวจจับเหตุการณ์ resize ของหน้าต่าง

    return () => {
      window.removeEventListener("resize", handleWindowResize); //ลบ event listener เพื่อป้องกันไม่ให้ถูกเรียก
    };
  });
